...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    ten_digit_oppo = opponent_score%100//10
    one_digit_play = player_score%10
    if(ten_digit_oppo-one_digit_play>0): 
        receive_point = 3*(ten_digit_oppo-one_digit_play)
    else: 
        receive_point =3*(-ten_digit_oppo+one_digit_play)
    if receive_point==0:
        return 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    result =0
    while n<=10:
        avrg=make_averaged(roll_dice,times_called)(n,dice)
        if avrg > max_average:
            max_average=avrg
            result=n
        n+=1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    potential_score =score +boar_brawl(score,opponent_score)
